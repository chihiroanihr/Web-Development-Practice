...
class IndexView(View):
    def get(self, request, *args, **kwargs):
        ...
        if profile_data.exists():
            profile_data = profile_data.order_by("-id")[0] # Get the most recent profile data (old one's are not shown)
...
class ContactView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            to_list = [email]
            bcc_list = [settings.EMAIL_HOST_USER] # Get notif to your host email when contact form received

            try:
                message = EmailMessage(subject=subject, body=content, to=to_list, bcc=bcc_list)
                message.send()
            except BadHeaderError:  # If send failed
                return HttpResponse("無効なヘッダが検出されました。")

            return redirect('thanks')

        # If form is invalid - start form from scratch
        return render(request, 'app/contact.html', {
            'form': form
        })
